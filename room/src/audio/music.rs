//! MUS-to-MIDI conversion and the music sample-rate constant.
//!
//! Pure, platform-shared pieces of the music stack.  This module replaces
//! Doom's original OPL2 music driver pipeline with a software-synthesiser
//! path: Doom ships music as either raw MIDI or DMX MUS lumps; MUS is
//! converted to a Standard MIDI File on the fly via [`mus2midi`] before
//! being handed to the platform audio backend's synthesiser (the native
//! shell renders it via `rustysynth`).
//!
//! ## Components
//!
//! - [`mus2midi`] — pure-Rust MUS-to-SMF converter.
//! - [`SAMPLE_RATE`] — the synthesiser output sample rate used by backends.

/// Audio output sample rate fed to the synthesiser.
...
/// for handing to the platform backend's music synthesiser.
///
...
pub fn mus2midi(data: &[u8]) -> Option<Vec<u8>> {
...
mod tests {
    ...
    fn mus2midi_rejects_bad_magic() {
        let data = b"MIDI\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
