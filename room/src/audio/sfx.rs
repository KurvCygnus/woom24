//! Sound-effect (SFX) decoding and panning helpers.
//!
//! Pure, platform-shared pieces of the audio stack: the decoder that turns
//! raw DMX `DSxxxxx` lumps into float-PCM samples, the conversion from
//! Doom's `(volume, separation)` pair into linear channel gains, and the
//! lock-free [`PanState`] shared between the game thread (which updates the
//! gains) and a platform backend's audio thread (which reads them while
//! mixing).  The mixer-graph types that consume these helpers live in the
//! platform shells (see `room-shell-native`'s audio backend).
//!
...
//!
//! The decoder converts the 8-bit samples to `f32` in `[-1, 1)`; backends
//! then upmix the mono stream to stereo, scaling each side by the gains
//! held in [`PanState`].
...
pub fn decode_doom_sfx(data: &[u8]) -> Option<(u32, Vec<f32>)> {
    if data.len() < 8 {
        return None;
    }
...
pub fn gains_from(vol: i32, sep: i32) -> (f32, f32) {
...
/// via [`PanState::update`]) and a platform backend's audio thread (which
/// reads the gains while mixing).
///
...
impl PanState {
    ...
    pub fn new(vol: i32, sep: i32) -> Self {
    ...
    pub fn update(&self, vol: i32, sep: i32) {
        let (l, r) = gains_from(vol, sep);
